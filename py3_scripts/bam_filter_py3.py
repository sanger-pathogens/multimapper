...
def main():
    ...
    parser.add_option("-b", "--bam", action="store", dest="bam",
                      help="input file in sam or bam format (must have suffix .sam or .bam)", default="",
                      metavar="FILE")
    parser.add_option("-o", "--output", action="store", dest="output", help="output prefix for file name(s)",
                      default="", metavar="FILE")
    parser.add_option("-f", "--format", type="choice", dest="fileformat",
                      choices=["fasta", "fastq", "pairedfastq", "sam", "bam"], action="store",
                      help="output file format (sam, bam, fastq, pairedfastq, or fasta) [default=%default]",
                      default="pairedfastq", metavar="FORMAT")
    parser.add_option("-t", "--type", action="store", type="choice", dest="outputtype",
                      choices=["unmapped", "mapped", "paired", "all", "contigs", "minusdup", "onemapped"],
                      help="reads to output (all, mapped, paired, unmapped, minusdup, onemapped or contigs). Note 1: for pairedfastq output, mapped includes all reads where one of the pair is mapped. paired includes only those reads where the reads are in a proper pair. For other output formats the mapped option will only print the individual mapped reads and paired requires the reads to be mapped in proper pairs. Note 2: the contigs option requires the -c option to be set (see below). [default=%default]",
                      default="unmapped", metavar="FILE")
    parser.add_option("-c", "--contigs", action="store", dest="contigs",
                      help="comma separated list of contigs for which mapped reads should be output. Note there should be no whitespace within the list",
                      default="")
...
if __name__ == "__main__":
    ...
    if options.fileformat == "fasta":
        ...
        output = pysam.AlignmentFile(options.output + ".sam", mode='wh', reference_names=newrefs, reference_lengths=newlengths)
    elif options.fileformat == "bam":
        output = pysam.AlignmentFile(options.output + ".bam", mode='wb', reference_names=newrefs, reference_lengths=newlengths)
    else:
    ...
    for read in samfile:
        ...
        elif not read.is_unmapped and (options.outputtype == "mapped" or (
                options.outputtype == "contigs" and refs[read.rname] in options.contiglist)):
            if options.fileformat == "pairedfastq":
        ...
        if printprev:
            if options.fileformat == "pairedfastq":
                print_read_to_file(output, prevtoprint, options.fileformat, sammate=firstofpair[prevname],
                                   mateout=routput)
                del firstofpair[prevname]
            else:
